...
/*
type RPBuffer []*Parser

func (rpb RPBuffer) Len() int {
    return len(rpb)
}

func (rpb RPBuffer) Less(i, j int) bool {
    return rpb[i].SequenceNumber() < rpb[j].SequenceNumber()
}

func (rpb RPBuffer) Swap(i, j int) {
    rpb[i], rpb[j] = rpb[j], rpb[i]
}

func (rpb *RPBuffer) Push(x interface{}) {
    // Push and Pop use pointer receivers because they modify the slice's length,
    // not just its contents.
    *rpb = append(*rpb, x.(*Parser))
}

func (rpb *RPBuffer) Pop() interface{} {
    old := *rpb
    n := len(old)
    x := old[n-1]
    *rpb = old[0 : n-1]
...
func NewRPBuffer() *RPBuffer {
	rpb := &RPBuffer{}
	heap.Init(rpb)
	return rpb
}

func (rpb *RPBuffer) HPush(rp *Parser) {
	heap.Push(rpb, rp)
}

func (rpb *RPBuffer) HPop() *Parser {
	return heap.Pop(rpb).(*Parser)
}*/